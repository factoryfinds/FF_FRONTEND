...
// components/MainScreen.tsx - Updated with better SEO structure
"use client";
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4">
            {categories.map((item, idx) => (
              <article
                key={item.slug}
                className={`group cursor-pointer relative w-full aspect-[3/4] overflow-hidden transition-all duration-300 ${
                  isLoading ? 'pointer-events-none opacity-75' : 'hover:scale-[1.01] hover:shadow-xl'
                }`}
                onClick={() => handleCategoryClick(item.slug)}
              >
                <Image
